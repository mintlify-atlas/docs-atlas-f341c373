...
description: Complete reference for integrating the claude-mem worker service into VSCode extensions, IDE plugins, and CLI tools
icon: plug
---
...
<CardGroup cols={2}>
  <Card title="Worker Base URL" icon="server">
  ...
  <Card title="Queue Observation" icon="database">
    `POST /api/sessions/observations`  
    Pass `claudeSessionId` + tool data
  </Card>
  <Card title="SSE Stream" icon="broadcast-tower">
    `GET /stream`  
...
{
  "sidebarOpen": boolean,
  "selectedProject": "my-project" | null
}
```
