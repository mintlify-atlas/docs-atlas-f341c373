...
The OpenClaw plugin gives persistent memory to agents running on the [OpenClaw](https://openclaw.ai) gateway. It handles three things:
...
<Steps>
  ...
  <Step title="Tool use recorded (tool_result_persist)">
    Every time the agent uses a tool (Read, Write, Bash, etc.), the plugin sends the observation to `POST /api/sessions/observations` with the tool name, input, and truncated response (max 1,000 chars). This is fire-and-forget — it does not block the agent from continuing work.
...
<ParamField body="observationFeed.botToken" type="string">
...
<ParamField body="observationFeed.emojis" type="object">
...
<CardGroup cols={2}>
  <Card title="Session Tracking" icon="timeline">
  ...
  <Card title="Context Cache" icon="database">
  ...
  <Card title="Fire-and-Forget HTTP" icon="bolt">
  ...
  <Card title="SSE Reconnection" icon="rotate">
